# -*- coding: utf-8 -*-
# @Time    : 21-4-24 下午3:54
# @File    : ST_LaneNet_back_end.py
# @IDE: PyCharm
"""
ST_LaneNet backend branch which is mainly used for binary and instance segmentation loss calculation
"""
import tensorflow as tf

from config import global_config
from ST_LaneNet_model import ST_LaneNet_discriminative_loss
from semantic_segmentation_zoo import cnn_basenet

CFG = global_config.cfg


class ST_LaneNetBackEnd(cnn_basenet.CNNBaseModel):
    """
    ST_LaneNet backend branch which is mainly used for binary and instance segmentation loss calculation
    """
    def __init__(self, phase):
        """
        init ST_LaneNet backend
        :param phase: train or test
        """
        super(ST_LaneNetBackEnd, self).__init__()
        self._phase = phase
        ...
        with tf.variable_scope(name_or_scope=name, reuse=reuse):
            ...
            with tf.variable_scope(name_or_scope='instance_seg'):
                ...
                instance_segmentation_loss, l_var, l_dist, l_reg = \
                    ST_LaneNet_discriminative_loss.discriminative_loss(
