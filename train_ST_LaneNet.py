# -*- coding: utf-8 -*-
# @Time    : 21-6-3 下午9:33
# @File    : train_ST_LaneNet.py
# @IDE: PyCharm
"""
Train ST_LaneNet script
"""
import argparse
import math
import os
...
from data_provider import ST_LaneNet_data_feed_pipline
...
def train_ST_LaneNet(dataset_dir, weights_path=None, net_flag='vgg'):
    """
    ...
    :return:
    """
    train_dataset = ST_LaneNet_data_feed_pipline.ST_LaneNetDataFeeder(
        dataset_dir=dataset_dir, flags='train'
    )
    val_dataset = ST_LaneNet_data_feed_pipline.ST_LaneNetDataFeeder(
        dataset_dir=dataset_dir, flags='val'
    )

    with tf.device('/gpu:1'):
        # set lanenet
        train_net = ST_LaneNet.ST_LaneNet(net_flag=net_flag, phase='train', reuse=False)
        val_net = ST_LaneNet.ST_LaneNet(net_flag=net_flag, phase='val', reuse=True)
        ...
        val_compute_ret = val_net.compute_loss(
            ...
            instance_label=val_instance_labels, name='ST_LaneNet_model'
        )
    ...
    model_save_dir = 'model/tusimple_ST_LaneNet_{:s}'.format(net_flag)
    ...
    tboard_save_path = 'tboard/tusimple_ST_LaneNet_{:s}'.format(net_flag)
...
def train_ST_LaneNet_multi_gpu(dataset_dir, weights_path=None, net_flag='vgg'):
    """
    train ST_LaneNet with multi gpu
    :param dataset_dir:
    :param weights_path:
    :param net_flag:
    :return:
    """
    # set ST_LaneNet dataset
    train_dataset = ST_LaneNet_data_feed_pipline.ST_LaneNetDataFeeder(
        dataset_dir=dataset_dir, flags='train'
    )
    val_dataset = ST_LaneNet_data_feed_pipline.ST_LaneNetDataFeeder(
    ...
    # Set tf summary save path
    tboard_save_path = 'tboard/tusimple_ST_LaneNet_multi_gpu_{:s}'.format(net_flag)
    os.makedirs(tboard_save_path, exist_ok=True)
    ...
    saver = tf.train.Saver()
    model_save_dir = 'model/tusimple_ST_LaneNet_multi_gpu_{:s}'.format(net_flag)
    os.makedirs(model_save_dir, exist_ok=True)
    train_start_time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
    model_name = 'tusimple_ST_LaneNet_{:s}_{:s}.ckpt'.format(net_flag, str(train_start_time))
    model_save_path = ops.join(model_save_dir, model_name)
    ...
    with sess.as_default():
        ...
        tf.train.write_graph(
            ...
            name='{:s}/ST_LaneNet_model.pb'.format(model_save_dir))
...
if __name__ == '__main__':
    ...
    if not args.multi_gpus:
        train_ST_LaneNet(args.dataset_dir, args.weights_path, net_flag=args.net_flag)
    else:
        train_ST_LaneNet_multi_gpu(args.dataset_dir, args.weights_path, net_flag=args.net_flag)
